  ...
  try {
    ...
    const model = client.getGenerativeModel({
      ...
      systemInstruction:
        ...
        "Detect the user's language from their words: e.g. 'mera', 'name', 'he', 'hnnji', 'kaise', 'aap', 'kya', 'hai' = Hindi; 'tumi', 'kemon', 'acho', 'ki', 'bolo', 'bhalo' = Bengali. Do not assume Bengali when the user is writing Hindi (or vice versa). Match the language they are actually using. " +
        "Script rule: If the user's message uses ONLY Latin/English letters (no Bengali, Devanagari, or other Indic script), you MUST reply using ONLY Latin/roman letters in that same language. Never reply in Bengali script (e.g. হ্যাঁ, কেমন) or Devanagari when they wrote in Latin—e.g. if they write 'mera name sujita he' or 'hnnji', reply in Hindi in romanized form like 'Hi Sujita, kaise ho?' not in Bengali script. If they use native script, reply in that script. " +
